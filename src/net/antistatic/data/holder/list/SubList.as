...
import net.antistatic.data.holder.list.AbstractList;
import net.antistatic.data.holder.list.ListIterator;
 ...
 * @author Simon Wacker
 */
public class SubList extends AbstractList implements IList {
	 ...
	 * {@code toIndex}
	 */
	public function SubList(list:IList, fromIndex:uint, toIndex:uint) {
		if (!list) throw new ArgumentError("Argument 'list' [" + list + "] must not be 'null' nor 'undefined'.");
		if (fromIndex < 0) throw new ArgumentError("Argument 'fromIndex' [" + fromIndex + "] must not be less than 0.");
		if (toIndex > list.size()) throw new ArgumentError("Argument 'toIndex' [" + toIndex + "] must not be greater than the size of the passed-in 'list' [" + list.size() + "].");
		if (fromIndex > toIndex) throw new ArgumentError("Argument 'fromIndex' [" + fromIndex + "] must not be greater than argument 'toIndex' [" + toIndex + "].");
		this.list = list;
	 ...
	 */
	public function set(index:uint, value:*):* {
		if (index < 0 || index >= size()) {
			throw new ArgumentError("Argument 'index' [" + index + "] is out of range, this is less than 0 or equal to or greater than this list's size [" + size() + "].");
		}
	 ...
	 */
	public function insertByIndexAndValue(index:uint, value:*):void {
		if (index < 0 || index > size()) {
			throw new ArgumentError("Argument 'index' [" + index + "] is out of range, this is less than 0 or greater than this list's size [" + size() + "].");
		}
	 ...
	 * equal to or greater than this list's size
	 */
	public function removeByIndex(index:uint):* {
		if (index < 0 || index >= size()) {
			throw new ArgumentError("Argument 'index' [" + index + "] is out of range, this is less than 0 or equal to or greater than this list's size [" + size() + "].");
		}
